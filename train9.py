# ============================================================
# DÉTECTEUR DE GAZ TOXIQUES — Random Forest + Multithreading
# ============================================================
...
def balance_humidity(dfX: pd.DataFrame, dfY: pd.DataFrame,
    ...
    # Reset index propre dès le départ
...
def train_model(model, X_train, y_train, label=''):
    print(f"  Entraînement {label}...")
    t0 = time.time()
    wrapped = MultiOutputRegressor(model, n_jobs=-1)
...
def run_pipeline(train_input_path, train_output_path, test_input_path,
    ...
    print("=" * 65)
    print("  DÉTECTEUR DE GAZ TOXIQUES  —  Random Forest + Multithreading")
    ...
    # ── [1/6] Chargement ────────────────────────────────────────────
    print("\n[1/6] Chargement parallèle des données...")
    ...
    # ── [4/6] Split ─────────────────────────────────────────────────
    print("\n[4/6] Split train / validation (80 / 20)...")
    X_tr, X_val, y_tr, y_val = train_test_split(
        X_all, y_mat, test_size=0.2, random_state=42
    )
    print(f"  Train : {len(X_tr)}  |  Validation : {len(X_val)}")

    # ── [5/6] Entraînement parallèle ────────────────────────────────
    print(f"\n[5/6] Entraînement des modèles...")

    def _train_rf():
        return train_model(build_rf(300), X_tr, y_tr, 'Random Forest')

    def _train_hgb():
        return train_model(build_hgb(), X_tr, y_tr, 'HistGradientBoosting')

    with ThreadPoolExecutor(max_workers=2) as exe:
        fut_rf  = exe.submit(_train_rf)
        fut_hgb = exe.submit(_train_hgb)
        rf_model  = fut_rf.result()
        hgb_model = fut_hgb.result()

    # ── Évaluation ──────────────────────────────────────────────────
    print("\n  Évaluation sur le set de validation...")
    scores = {}
    for name, model in [('Random Forest', rf_model), ('HGB', hgb_model)]:
        pred  = safe_predict(model, X_val.copy())
        score = weighted_rmse(y_val, pred)
        scores[name] = score
        print(f"  {name:<25} Weighted RMSE : {score:.6f}")

    best_name  = min(scores, key=scores.get)
    best_model = rf_model if best_name == 'Random Forest' else hgb_model
    print(f"\n  ✔ Meilleur modèle : {best_name} ({scores[best_name]:.6f})")

    # Feature importances RF
    try:
        importances = np.mean(
            [e.feature_importances_ for e in rf_model.estimators_], axis=0
        )
    ...
    # ── [6/6] Prédictions ───────────────────────────────────────────
    print("\n[6/6] Génération des prédictions sur le test set...")
    test_preds = safe_predict(best_model, X_test.copy())
